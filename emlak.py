...
hedef_link = [
    ...
    "https://www.sahibinden.com/satilik-daire/canakkale-merkez/sahibinden?pagingSize=50&sorting=price_asc"
]
...
def temel_verileri_sayisallastir(ozellikler, ilan_id, temiz_fiyat, link, soup):
    ...
    def yas_hesapla_ordinal(metin):
        """
        Ham yaş kategorisini ordinal bir alana çevirir.
        Daha yeni bina -> daha yüksek skor
        """
        metin = str(metin).lower().strip()

        if "sıfır" in metin or metin == "0":
            return 5.0
        elif "0-5" in metin:
            return 4.0
        elif "6-10" in metin:
            return 3.0
        elif "11-15" in metin or "11-25" in metin:
            return 2.0
        elif "26" in metin or "üzeri" in metin:
            return 1.0
    ...
    def yas_hesapla_numeric(metin):
        """
        Mümkünse gerçek sayısal yaş yakalamaya çalışır.
        Kategori gelirse yaklaşık temsil döner.
        """
        metin = str(metin).lower().strip()
    ...
    def kat_hesapla_ordinal(metin):
        """
        Yorum içeren ama ek feature olarak tutulabilecek ordinal skor.
        Ham kat bilgisinin yerine geçmez.
        """
        metin = str(metin).lower().strip()
    ...
    def kat_no_hesapla(metin):
        """
        Mümkünse gerçek sayısal kat no.
        Kategori ifadeleri için yaklaşık temsil kullanır.
        """
        metin = str(metin).lower().strip()

        if "bodrum" in metin:
            return -1.0
        if "giriş altı" in metin or "kot" in metin:
            return -0.5
        if "bahçe" in metin:
            return 0.0
        if "zemin" in metin:
    ...
    def isitma_puani_hesapla(metin):
        """
        Bu alan yorum içerir; ham ısıtma tipi de ayrıca tutulmalı.
        """
        metin = str(metin).lower().strip()
        taban_puan = 0.0

        if (
            "kombi (doğalgaz)" in metin or
            "yerden" in metin or
            "pay ölçer" in metin or
            "ısı pompası" in metin or
            "jeotermal" in metin
        ):
            taban_puan = 5.0
            ...
            "doğalgaz sobası" in metin or
            "elektrikli radyatör" in metin or
            "şömine" in metin or
            "güneş enerjisi" in metin
        ):
            taban_puan = 2.0
        ...
        if "klima" in metin:
            if taban_puan >= 4.0:
                ...
                taban_puan = max(taban_puan, 3.0)

        return taban_puan if taban_puan > 0 else 0.0

    def isitma_ana_sinif(metin):
        """
        Isıtmayı daha kaba ama daha kararlı kategorik sınıfa indirger.
        """
        metin = str(metin).lower().strip()
        ...
        if "merkezi" in metin:
            return "Merkezi"
        if "kombi" in metin and "doğalgaz" in metin:
            return "Kombi Dogalgaz"
        if "kombi" in metin and "elektrik" in metin:
            return "Kombi Elektrik"
        if "kat kaloriferi" in metin:
            return "Kat Kaloriferi"
        if "vrv" in metin:
            return "VRV"
        if "fancoil" in metin:
            return "Fancoil"
        if "ısı pompası" in metin:
            return "Isi Pompası"
        if "jeotermal" in metin:
            return "Jeotermal"
        if "doğalgaz sobası" in metin:
...
for index, link in enumerate(tum_ilan_linkleri):
    ...
    # --- YENİ EKLENEN: HAFIZA VE ÇİFT KAYIT KONTROLÜ ---
    # Linke girmeden önce URL'den ID'yi kopar
    match = re.search(r'-(\d+)(?:/detay)?$', link)
    ilan_id_test = match.group(1) if match else None
    
    # Eğer bu ID'ye ait bir klasör zaten varsa, bu ilanı daha önce çekmişiz demektir!
    if ilan_id_test:
        ...
        if os.path.exists(test_klasoru):
            ...
            continue # Aşağıdaki hiçbir kodu çalıştırmadan bir sonraki ilana geç
    # --------------------------------------------------
    
    # --- YENİ ANTİ-BAN KORUMASI: KAHVE MOLASI ---
    ...
    gercekci_kaydir(driver) # İlan içinde insan gibi gezin
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    ...
    try:
        ...
        # YENİ DİNAMİK KLASÖR VE DOSYA YOLU MANTIĞI
